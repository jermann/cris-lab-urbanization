...
households-own [
  ...
  weight-density
  weight-school
  weight-attractive2015
  weight-healthfacility
]
;;.................................................
...
to make-households
  ...
  ;; AJ
  ask households [
    set weight-bu2010 random-float 1
    set weight-cbd random-float 1
    set weight-shoppingmall random-float 1
    set weight-market random-float 1
    set weight-road random-float 1
    set weight-density random-float 1
    set weight-school random-float 1
    set weight-attractive2015 random-float 1
    set weight-healthfacility random-float 1
  ]
...
to make-developers
  ...
  ask developers [
    set weight-road random-float 1
    set weight-suburb random-float 1
    set weight-attractive2015 random-float 1
    set weight-slope random-float 1
    set weight-school random-float 1
  ]
...
to make-low-income-household-development-old
  let i household-number
...
to make-low-income-household-development
  ...
  let d patches with [(urban = 0) and (new-urban = 0) and (suitability = 1) and (landvalue < 2) and (LI-movement = 0) and (HI-movement = 0) and
      (MI-movement = 0) and (pMED-movement = 0) and (pHED-movement = 0)]

  ;; Assume the agent evaluates all patches; adjust the criteria as needed
  let available-patches patches with [suitability = 1 and urban = 0]

  let patch-utilities []

  foreach d [
    patch-temp ->
    let h-utility calc-utility patch-temp
    set patch-utilities lput h-utility patch-utilities

  ]
  ;; Make a decision based on calculated utilities, e.g., move to the highest utility patch
  let best-patch item max-n-of 1 d [y -> item position y patch-utilities]  ;; Assuming the agent prefers the patch with the highest utility
  move-to best-patch
  ask patch-here
  [(set pcolor yellow) (set LI-movement 1) (set new-urban 1)]
...
to-report calc-utility [target-patch]
  ;; Calculate and return utility based on the agent's weights and the attributes of 'target-patch'
   report (((1 - [suburb] of target-patch) * weight-suburb) +
          (([nom-neighbourhood] of target-patch) * weight-neighbourhood) +
          ((1 - [bu2010] of target-patch) * weight-bu2010) +
          ((1 - [cbd] of target-patch) * weight-cbd) +
          ((1 - [shoppingmall] of target-patch) * weight-shoppingmall) +
          ((1 - [market] of target-patch) * weight-market) +
          ((1 - [road] of target-patch) * weight-road) +
          ([density] of target-patch * weight-density) +
          ((1 - [school] of target-patch) * weight-school) +
          ((1 - [attractive2015] of target-patch) * weight-attractive2015) +
          ((1 - [healthfacility] of target-patch) * weight-healthfacility) +
          (random-float 0.1))
end


;; ULI (((1 - suburb) * 10) + (nom-neighbourhood * 7) + ((1 - bu2010) * 9) + ((1 - cbd) * 5) + (1 - shoppingmall) +
;;      ((1 - market) * 6) + ((1 - road) * 8) + (density * 2) + ((1 - school) * 2) + ((1 - attractive2015) * 2) + (1 - healthfacility) + (random-float 0.1))
