...
to export-output-income-status
  ...
  file-open "Output/Income_status/urban_expansion_income_all_income_equal.asc"
...
to export-output-legal-status
  ...
  file-open "Output/formality_status/formality_all_income_equal.asc"
  file-print "ncols         886   \r\n"
  file-print "nrows         536   \r\n"
  file-print "xllcorner     730915.48888016   \r\n"
  file-print "yllcorner     577823.10587025 \r\n"
...
to export-utility-data
  file-open "Output/utility_data_all_income_equal.asc"
  ...
  while [i > -1]
    [ set rows []
      set rows patches with [pycor = i]
        foreach sort-on [pxcor] rows [ ?1 -> ask ?1 [
         let utility-value 0
         (ifelse pcolor = yellow [ set utility-value ULI ] ;; low income
                 pcolor = orange [ set utility-value UMI ] ;; middle income
                 pcolor = blue   [ set utility-value UHI ] ;; high income 
                 pcolor = green  [ set utility-value 0   ] ;; undeveloped lands 
                 pcolor = brown  [ set utility-value 0   ] ;; seed year built-up 
          [ set utility-value 0 ])
          file-write utility-value
      ] ]
     file-print "   "
     set i i - 1]
